...
from api_models import GenerationRequest, MultiViewGenerationRequest, GenerationResponse, StatusResponse, HealthResponse
...
@app.post("/generate-multiview", tags=["generation"])
async def generate_3d_model_multiview(request: MultiViewGenerationRequest):
    """
    Generate a 3D model from multiple view images.
    
    This endpoint takes multiple view images (front, back, left, right) and generates a 3D model with optional textures.
    At least one view image must be provided. The generation process includes background removal, mesh generation, and optional texture mapping.
    
    Returns:
        FileResponse: The generated 3D model file (GLB or OBJ format)
    """
    logger.info("Worker generating multiview...")
    
    # Validate that at least one view is provided
    if not request.images or len(request.images) == 0:
        ret = {
            "text": "At least one view image must be provided",
            "error_code": 1,
        }
    ...
    uid = uuid.uuid4()
    try:
        file_path, uid = worker.generate_multiview(uid, params)
        return FileResponse(file_path)
    except ValueError as e:
        traceback.print_exc()
        logger.error(f"Caught ValueError: {e}")
...
@app.post("/send-multiview", response_model=GenerationResponse, tags=["generation"])
async def send_multiview_generation_task(request: MultiViewGenerationRequest):
    """
    Send a multi-view 3D generation task to be processed asynchronously.
    
    This endpoint starts the multi-view generation process in the background and returns a task ID.
    Use the /status/{uid} endpoint to check the progress and retrieve the result.
    
    Returns:
        GenerationResponse: Contains the unique task identifier
    """
    logger.info("Worker send multiview...")
    ...
    uid = uuid.uuid4()
    try:
        threading.Thread(target=worker.generate_multiview, args=(uid, params,)).start()
        ret = {"uid": str(uid)}
        return JSONResponse(ret, status_code=200)
    except Exception as e:
        logger.error(f"Failed to start multiview generation thread: {e}")
        ret = {"error": "Failed to start multiview generation"}
        return JSONResponse(ret, status_code=500)
...
async def status(uid: str):
    ...
    # If only initial file exists, check if this is a shape-only generation
